...
from rest.serializers import (
    ProductoSerializer,
    LoteSerializer,
    SalidaLoteSerializer,
    EntradaLoteSerializer,
)
...
def getLotesByInvima(request):
    ...
    if len(lotesByInvima) == 0:
        return Response(
            "No lotes disponibles para este articulo.",
            status=status.HTTP_404_NOT_FOUND,
        )
    else:
...
def getSalidaLotesByInvima(request):
    ...
    if len(salidaLotesByInvima) == 0:
        return Response(
            "Este lote no ha presentado salidas.",
            status=status.HTTP_404_NOT_FOUND,
        )
    else:
...
def getEntradaLotesByInvima(request):
    invima = request.data["invima"]
    entradaLotesByInvima = EntradaLote.objects.filter(
        producto__producto=invima
    )
    if len(entradaLotesByInvima) == 0:
        return Response(
            "Este lote no ha presentado entradas.",
            status=status.HTTP_404_NOT_FOUND,
        )
    else:
...
def registrarEntradas(request):
    if request.method == "POST":
        if request.data:
            ...
            return Response(
                "Datos incorrectos.", status=status.HTTP_400_BAD_REQUEST
            )
    ...
    if request.method == "POST":
        if request.data:
            for salida in request.data:
                ...
                SalidaLote.create(
                    salida["cantidad_salida"], producto, salida["descripcion"]
                )
            return Response("Success", status=status.HTTP_201_CREATED)
        else:
            return Response(
