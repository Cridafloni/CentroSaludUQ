...
class ProductoAdmin(ImportExportModelAdmin, SummernoteModelAdmin):
    ...
    list_per_page = 30
    fields = ('tipo', 'nombre',
              'presentacion', 'material', 'registro_invima',
    ...
    def descargar_base_producto(self, request, queryset):
        ...
        response['Content-Disposition'] = "attachment; filename=BaseProducto.xlsx"
        return response

    descargar_base_producto.short_description = "Descargar el archivo Base o plantilla."

    def descargar_registros(self, request, queryset):
        lista = list(queryset)
        finalList = []
        finalList.append(["Nombre", "Presentacion", "Proveedor",
                          "Registro Invima", "Tipo", "Uds Disponibles"])
        for producto in lista:
            uno = [producto.nombre, producto.presentacion, producto.proveedor,
                   producto.registro_invima,
                   producto.tipo, producto.cantidad_unidades_disponibles]
...
class LoteAdmin(ImportExportModelAdmin, SummernoteModelAdmin):
    ...
    descargar_base_lote.short_description = "Descargar el archivo Base o plantilla."

    def descargar_registros(self, request, queryset):
        lista = list(queryset)
        finalList = []
        finalList.append(["Producto", "Proveedor", "Presentacion", "Registro Invima",
                          "Fecha de caducidad", "Unidades", "Meses por vencer", "Estado"])
        for lote in lista:
            uno = [lote.producto.nombre, lote.producto.proveedor, lote.producto.presentacion,
                   lote.producto.registro_invima, lote.fecha_vencimiento, lote.unidades_restantes,
                   lote.meses_vencimiento, lote.estado]
            finalList.append(uno)
        # List of Lists
        data = finalList
...
class SalidaLoteAdmin(ImportExportModelAdmin):
    ...
    list_display = ('producto', 'fecha_salida', 'cantidad_salida', 'descripcion')
    list_filter = ('producto', 'fecha_salida')
    ordering = ('fecha_salida',)
    date_hierarchy = 'fecha_salida'
    search_fields = ('producto',)
    list_per_page = 30
    fields = ('cantidad_salida',
    ...
    def descargar_registros(self, request, queryset):
        lista = list(queryset)
        finalList = []
        finalList.append(["Producto", "Cantidad salida", "Fecha salida", "Descripcion"])
        for salidaLote in lista:
            uno = [
                salidaLote.producto.producto.nombre + " - " + salidaLote.producto.producto.presentacion + " - " + salidaLote.producto.lote_del_producto,
                salidaLote.cantidad_salida, salidaLote.fecha_salida, salidaLote.descripcion]
            finalList.append(uno)
        # List of Lists
        data = finalList
...
class EntradaLoteAdmin(ImportExportModelAdmin):
    list_display = ('producto', 'fecha_entrada', 'cantidad_entrante', 'descripcion')
    list_filter = ('fecha_entrada', 'producto')
    ordering = ('fecha_entrada',)
    date_hierarchy = 'fecha_entrada'
    search_fields = ('producto',)
    list_per_page = 30
    fields = ('cantidad_entrante',
              'producto', 'descripcion')
    raw_id_fields = ('producto',)
    ...
    def descargar_registros(self, request, queryset):
        lista = list(queryset)
        finalList = []
        finalList.append(["Producto", "Cantidad entrada", "Fecha entrada", "Descripcion"])
        for entradaLote in lista:
            uno = [
                entradaLote.producto.producto.nombre + " - " + entradaLote.producto.producto.presentacion + " - " + entradaLote.producto.lote_del_producto,
                entradaLote.cantidad_entrante, entradaLote.fecha_entrada, entradaLote.descripcion]
            finalList.append(uno)
        # List of Lists
        data = finalList
...
def pdfConfig(data, nombre):
    ...
    header = Paragraph("<bold><font size=18>{}</font></bold>".format(nombre), style)
    t = Table(data)
